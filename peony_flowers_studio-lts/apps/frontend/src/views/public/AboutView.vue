<template>
  <div class="page container">
    ...
    <template v-else-if="content">
      ...
      <section v-if="storeLat && storeLng" class="section">
        <div class="location-header">
          <h2 class="headline-sm section-title">{{ $t('about.locationTitle') }}</h2>
          <p v-if="content.address" class="body-md address">{{ content.address }}</p>
        </div>
        <StoreMap :lat="storeLat" :lng="storeLng" :address="content.address" />
      </section>
    </template>
...
// Do'kon manzili: avval backenddan kelgan content.lat/lng, bo'lmasa
// .env orqali berilgan standart koordinata ishlatiladi.
const storeLat = computed(
  () => content.value?.lat ?? Number(import.meta.env.VITE_STORE_LAT ?? NaN)
);
const storeLng = computed(
  () => content.value?.lng ?? Number(import.meta.env.VITE_STORE_LNG ?? NaN)
);
</script>
