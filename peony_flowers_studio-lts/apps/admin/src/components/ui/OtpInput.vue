...
function onInput(index: number, event: Event) {
    ...
    if (value && index < len - 1) {
        const next = document.getElementById(`admin-otp-${index + 1}`);
...
function onKeydown(index: number, event: KeyboardEvent) {
    if (event.key === 'Backspace' && !digits.value[index] && index > 0) {
        const prev = document.getElementById(`admin-otp-${index - 1}`);
...
<template>
    <div class="otp">
        <input v-for="(d, i) in digits" :id="`admin-otp-${i}`" :key="i" v-model="digits[i]" maxlength="1"
            inputmode="numeric" @input="onInput(i, $event)" @keydown="onKeydown(i, $event)" />
    </div>
</template>
