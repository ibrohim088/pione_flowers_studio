    ...
    <template v-else>
      ...
      <div v-for="locale in (['uz', 'ru'] as const)" :key="locale" v-show="activeTab === locale" class="form">
        ...
        <div class="list-field">
          ...
          <AppButton variant="outline" type="button" @click="addListItem(locale, 'services')">+ Xizmat qo'shish</AppButton>
        </div>
        ...
        <div class="list-field">
          ...
          <AppButton variant="outline" type="button" @click="addListItem(locale, 'values')">+ Qadriyat qo'shish</AppButton>
        </div>
      </div>
    </template>
  </div>
...
.subtitle { color: var(--text-secondary); font-size: 13px; max-width: 480px; margin-top: 6px; }
.header-actions { display: flex; align-items: center; gap: 12px; flex-shrink: 0; }
.status { font-size: 13px; color: #2a9d5c; white-space: nowrap; &.error { color: #d33; } }
.hint {
  background: var(--accent-light); color: var(--accent); padding: 12px 16px;
  border-radius: var(--radius); font-size: 13px; margin: 16px 0;
}
...
.form { display: flex; flex-direction: column; gap: 18px; padding-top: 4px; }
.row { display: flex; gap: 16px; }
.row > * { flex: 1; }

.list-field { display: flex; flex-direction: column; gap: 8px; }
.list-label { font-size: 12px; font-weight: 500; color: var(--text-secondary); }
.list-row { display: flex; align-items: center; gap: 8px; }
.list-row > :first-child { flex: 1; }
.remove-btn {
  border: 1px solid var(--border); background: var(--bg-primary); border-radius: var(--radius);
  width: 34px; height: 34px; flex-shrink: 0; cursor: pointer; color: var(--text-secondary);
  &:hover { color: #d33; border-color: #d33; }
}
</style>
